...
4. **Given** 使用者正確填寫所有資訊, **When** 點擊「確認訂單」, **Then** 產生唯一訂單編號（格式：ORD{yyyyMMddHHmmss}），訂單資料儲存至系統
...
當使用者有進行中的訂單（狀態為待確認、已確認或準備中）時，在餐廳列表頁面頂部顯示提醒區塊，包含訂單編號、餐廳名稱、訂單狀態和「查看訂單詳情」按鈕。
...
2. **Given** 提醒區塊顯示, **When** 使用者查看, **Then** 顯示訂單編號、餐廳名稱、訂單狀態（待確認/已確認/準備中）
...
- 當使用者在結帳頁面停留過久（超過30分鐘），訂單摘要可能失效，系統提示「訂單已逾時，請重新選擇菜品」
...
- **FR-007**: 系統必須提供「加入訂單」功能，將選定的菜品和數量加入訂單摘要
- **FR-008**: 系統必須在訂單摘要區塊即時顯示已選菜品清單、數量、小計和訂單總金額（格式：NT$ 1,234.00）
...
- **FR-015**: 系統必須在使用者確認訂單後產生唯一訂單編號（格式：ORD{yyyyMMddHHmmss}）
- **FR-016**: 系統必須將訂單資料持久化儲存，包含訂單編號、餐廳資訊、訂餐者資訊、訂單項目、總金額、訂單狀態和建立時間
- **FR-017**: 系統必須在訂單成功提交後顯示訂單編號、「返回首頁」和「查看訂單紀錄」連結
- **FR-018**: 系統必須提供訂單紀錄頁面，顯示最近5天內的訂單清單（包含訂單編號、日期、餐廳名稱、總金額、狀態）
- **FR-019**: 系統必須自動清理超過5天的訂單資料
...
- **FR-023**: 系統必須提供「返回修改」功能，允許使用者從結帳頁面返回菜單頁面，並保留當前已選擇的菜品
...
- **Order（訂單）**: 代表一筆完整的訂單，包含唯一訂單編號、餐廳資訊（ID和名稱快照）、訂餐者資訊（姓名和電話）、訂單項目清單、訂單總金額、訂單狀態（待確認/已確認/準備中/已完成/已取消）和建立時間
- **OrderItem（訂單項目）**: 代表訂單中的單一菜品項目，包含菜品ID、菜品名稱快照、單價快照、訂購數量和小計（單價 × 數量）
- **OrderStatus（訂單狀態）**: 定義訂單的狀態，包含待確認（Pending）、已確認（Confirmed）、準備中（Preparing）、已完成（Completed）、已取消（Cancelled）
- **Restaurant（餐廳）**: 代表可訂餐的餐廳，包含餐廳ID、名稱、地址、聯絡電話、營業時間和菜單項目清單（關聯至MenuItems）
